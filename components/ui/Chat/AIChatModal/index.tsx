    ...
    <div className="flex gap-2.5">
      ...
      <div className="bg-white rounded-2xl rounded-tl-md px-4 py-4 shadow-[0_2px_12px_rgba(13,48,128,0.06)] max-w-[88%] space-y-3">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-1">아래 내용을 편하게 말씀해주세요</p>
        </div>
        <div className="grid grid-cols-2 gap-1.5">
          {INTRO_ITEMS.map(({ Icon, label, desc, required }) => (
            <div key={label} className="flex items-center gap-2 bg-gradient-to-br from-[#f6f8ff] to-[#fdf6f9] rounded-xl px-2.5 py-2 border border-gray-100/80">
              ...
              <div className="min-w-0">
                <span className="text-[11px] font-semibold text-gray-700 leading-none">
                  {label}{required && <span className="text-[#FF6B9D] ml-0.5">*</span>}
                </span>
...
function MessageBubble({ role, content, status }: { role: 'user' | 'assistant'; content: string; status?: string }) {
  ...
  return (
    <div className={`flex gap-2.5 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`max-w-[78%] px-3.5 py-2.5 rounded-2xl text-[13px] leading-relaxed ${
        isUser ? 'bg-gradient-to-br from-[#0d3080] to-[#2456d6] text-white rounded-tr-md shadow-[0_2px_8px_rgba(13,48,128,0.25)]'
        : isCompleted ? 'bg-gradient-to-br from-[#fff0f5] to-[#fff7fa] border border-[#FF6B9D]/20 text-gray-800 rounded-tl-md'
        : isOffTopic ? 'bg-[#fffbf0] border border-amber-200/50 text-gray-600 rounded-tl-md shadow-sm'
        : 'bg-white text-gray-700 rounded-tl-md shadow-[0_2px_12px_rgba(13,48,128,0.06)]'
      }`}>
        {isCompleted && <p className="text-[#FF6B9D] font-bold text-[11px] mb-1 flex items-center gap-1"><Sparkles className="w-3 h-3" /> 코스 생성 완료!</p>}
  ...
  return (
    <div className="flex gap-2.5">
      ...
      <div className="bg-white rounded-2xl rounded-tl-md px-4 py-3 shadow-[0_2px_12px_rgba(13,48,128,0.06)] flex items-center gap-1.5">
        {[0, 1, 2].map((i) => (
          <span key={i} className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.15}s`, background: 'linear-gradient(135deg, #0d3080, #FF6B9D)' }} />
        ))}
      </div>
    </div>
  );
...
export function AIChatModal({ isOpen, onClose, initialMessage }: AIChatModalProps) {
  ...
  // 열릴 때 initialMessage가 있으면 input에 채워줌
  useEffect(() => {
    if (isOpen) {
  ...
  const handleReset = () => { reset(); inputRef.current?.focus(); };

  if (!isOpen) return null;

  return (
    <>
      <div className="fixed inset-0 bg-[#0d3080]/20 backdrop-blur-[3px] z-40" onClick={onClose} />
      <div
        className="fixed z-50 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-[440px] max-w-[calc(100vw-32px)] h-[600px] max-h-[calc(100vh-48px)] flex flex-col rounded-[20px] overflow-hidden shadow-[0_24px_64px_rgba(13,48,128,0.25)]"
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 bg-gradient-to-r from-[#0d3080] via-[#1a44b8] to-[#2456d6]">
          ...
          <div className="flex items-center gap-0.5">
            {messages.length > 0 && (
              <button onClick={handleReset} className="p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/10 transition-colors" title="새 대화">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3.5">
          <IntroMessage />
          {messages.map((msg) => <MessageBubble key={msg.id} role={msg.role} content={msg.content} status={msg.status} />)}
          {isLoading && <TypingIndicator />}
          {isAwaitingConfirmation && !isLoading && (
            <div className="flex justify-center gap-2 pt-1">
              <button onClick={handleReset} className="px-4 py-2 rounded-full text-[12px] font-semibold text-gray-500 bg-white border border-gray-200 shadow-sm hover:bg-gray-50 active:scale-95 transition-all">
                🔄 처음부터 다시
              </button>
              <button onClick={sendConfirmation} className="px-4 py-2 rounded-full text-[12px] font-bold bg-gradient-to-r from-[#FF6B9D] to-[#ff4d8f] text-white shadow-[0_4px_14px_rgba(255,77,143,0.4)] hover:brightness-105 active:scale-95 transition-all">
                ✨ 네, 이대로 만들어주세요!
              </button>
            </div>
          )}
        ...
        <div className="flex-shrink-0 px-3 pb-3 pt-2" style={{ background: '#F7F8FC' }}>
          <div className="flex items-center gap-2 bg-white rounded-2xl px-4 py-2.5 shadow-[0_2px_12px_rgba(13,48,128,0.08)] border border-gray-100 focus-within:border-[#0d3080]/30 focus-within:shadow-[0_2px_16px_rgba(13,48,128,0.12)] transition-all">
            ...
            <button
              ...
              className="flex-shrink-0 w-8 h-8 rounded-xl bg-gradient-to-br from-[#0d3080] to-[#2456d6] flex items-center justify-center text-white shadow-[0_2px_8px_rgba(13,48,128,0.3)] hover:brightness-110 active:scale-90 disabled:opacity-30 disabled:cursor-not-allowed transition-all self-end"
            >
          ...
          <div className="flex items-center justify-between mt-1.5 px-1.5 min-h-[14px]">
            ...
            {inputValue.length > MAX_INPUT_LENGTH * 0.7 && (
              <span className={`text-[10px] tabular-nums ${inputValue.length >= MAX_INPUT_LENGTH ? 'text-red-400' : 'text-gray-400'}`}>
